...
# Calculate mean algae cover in each treatment for 2019
algae_totals %>% 
  filter(time_point == "2019") %>% 
  group_by(treatment) %>% 
  summarize(mean_cover.prop = mean(total_cover.prop),
            SE = sd(total_cover.prop)/sqrt(n()),
            CI = SE*2)
...
## Stacked area chart with proportions of algae rather than total percent cover
algae_prop <- algae %>%
...
algae_means.coral_change %>%
  ...
  geom_point(aes(x = algae.mean*100, y = delta_coral, shape = treatment),
             alpha = 0.6,
             size = 2)+
  labs(x = expression(Mean~percent~cover~of~macroalgae~(t[N]~to~t[N+1])),
       y = expression(Proportional~live~coral~change~(t[N]~to~t[N+1]))) +
  scale_shape_manual(values = c(19,17),
                     labels = c("Removal", "Retention"),
                     name = "Skeletal treatment") +
  theme_classic(base_size = 14)
...
ggplot(recruit_counts.summary, aes(x = treatment, y = proportion, fill = substrate)) +
  ...
  scale_fill_manual(values = c("#0B4221", "#9BB1BB"),
                    name = "Substrate type") +
...
ggplot(dead.2023.summary, aes(x = treatment, y = mean_surf.m_sq, shape = treatment)) +
  ...
  geom_point(size = 2,
             color = "#CACFD0") +
  labs(x = "Skeleton treatment",
       y = "Mean surf. area (m^2) of dead coral (± SE)") +
  scale_shape_manual(values = c(19,17),
